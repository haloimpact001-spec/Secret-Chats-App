...
export async function generateKey(): Promise<CryptoKey> {
  return window.crypto.subtle.generateKey({ name: ALGORITHM, length: KEY_LENGTH }, true, ['encrypt', 'decrypt']);
}
...
export async function importKeyFromBase64(base64Key: string): Promise<CryptoKey> {
  ...
  return window.crypto.subtle.importKey('raw', bytes, { name: ALGORITHM, length: KEY_LENGTH }, true, ['encrypt', 'decrypt']);
}
...
export async function decryptText(base64Payload: string, key: CryptoKey): Promise<string> {
  ...
  const decryptedBuffer = await window.crypto.subtle.decrypt({ name: ALGORITHM, iv }, key, ciphertext);
